...
def sample_constrained_p0(tsp, shots, seed=None):

    N = tsp.N
    n = tsp.ising.h.shape[0]  # should be N*N
    ...
    for _ in range(shots):
        ...
        # -------------------------------------------------
        # Sample x directly from constrained initial state
        # -------------------------------------------------
        x = np.zeros(n, dtype=int)

        for t in range(N):
            i = rng.integers(0, N)  # pick city uniformly
        ...
        ###
            ...
            for s_idx, shots in enumerate(shots_list):
                ...
                try:
                    ...
                    # -------------------------
                    if p == 0:
                        route_qaoa, cost_qaoa = sample_constrained_p0(tsp, shots)
